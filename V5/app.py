...
init_feedback_db()
@app.post("/upload-schemes-csv")
async def upload_schemes_csv(file: UploadFile = File(...)):
    ...
    for row in reader:
        ...
        c.execute('''INSERT INTO schemes (organization, program, focus_area, grant_amount, deadline, link)
                     VALUES (?, ?, ?, ?, ?, ?)''', tuple(row))
    
    conn.commit()
    conn.close()

    return JSONResponse(content={"message": "CSV uploaded and schemes added successfully!"})
@app.get("/schemes")
async def get_schemes():
    ...
    return {"schemes": [dict(zip(["id", "organization", "program", "focus_area", "grant_amount", "deadline", "link"], scheme)) for scheme in schemes]}
@app.post("/add-scheme")
async def add_scheme(request: Request):
    ...
    c.execute('''INSERT INTO schemes (organization, program, focus_area, grant_amount, deadline, link)
                 VALUES (?, ?, ?, ?, ?, ?)''',
              (organization, program, focus_area, grant_amount, deadline, link))
    conn.commit()
    conn.close()

    return JSONResponse(content={"message": "Scheme added successfully!"})
...
async def submit_feedback(request: Request):
    ...
    conn = sqlite3.connect("startup_classifier.db")
    c = conn.cursor()
    c.execute('''INSERT INTO feedback (timestamp, email, message)
                 VALUES (?, ?, ?)''', (timestamp, email, message))
    conn.commit()
    conn.close()
...
async def view_feedback(request: Request):
    conn = sqlite3.connect("startup_classifier.db")
    c = conn.cursor()
    c.execute('''SELECT timestamp, email, message FROM feedback ORDER BY timestamp DESC''')
